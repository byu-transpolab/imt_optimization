...
-   A wide span across the plots for both groupings of Incidents, indicating a significant variance in delays across those simulations.
...
From @tbl-impacted_links, it's clear that, in line with other VHD comparisons, delay in the 30 IMT group most closely mirrors the baseline. Followed by the 20 IMT scenarios and then the Incidents scenarios. Comparing the Total VHD of Current and Increased Incident groups would be misleading, because they are based on different sample sizes of incident as noted in @tbl-impacted_links. A more meaningful comparison might be the average delay per incident. By that metric, we see that the average VHD increased for IMT groups when a higher number of incidents were simulated. Conversely, comparing the Incident scenarios is less straightforward, where both Total VHD and average delay per incident were higher in the scenarios with less incidents. Examining the data more closely, we see that one scenario in particular (Seed 141) produced abnormally large delay along incident links, and may account for the odd results observed in the No IMT current Incident frequency scenarios. 
