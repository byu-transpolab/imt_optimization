  ...
  make_truck_time_plot <- function(truck_time) {
    ...
    # make_truck_distance_table <- function(truck_time) {
    #   # Reduce to unique combinations of Seed, Scenario, Average Time, and Incidents
    #   truck_distance_data <- truck_time %>%
    #     select(Seed, Scenario, `Average.Distance..meters.`, Incidents) %>%
    #     distinct()
    #   
    #   # Convert the average distance to miles
    #   truck_distance_data$`Average.Distance..miles.` <- truck_distance_data$`Average.Distance..meters.` / 1609.344
    #   
    #   # Create an ordered vector of Seeds based on Scenario 3 Average Travel Distances
    #   ordered_seeds <- truck_distance_data %>%
    #     filter(Scenario == "Increased") %>%
    #     arrange(`Average.Distance..miles.`) %>%
    #     pull(Seed)
    #   
    #   # Reorder Seed factor levels in df_unique based on ordered_seeds
    #   truck_distance_data$Seed <- factor(truck_distance_data$Seed, levels = ordered_seeds)
    #   
    #   truck_distance_table <- truck_distance_data
    #   
    #   truck_distance_table
    # }
