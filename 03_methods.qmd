...
Twenty values in total were selected, with ten being part of the Current Incident Frequency grouping and the other ten as part of the Increased Incident Frequency grouping. Each of these value was then assigned a three digit seed number which was used internally within MATSim to randomize the incident selection for for each scenario. The [Network Change Events](#sec-NCE) MATSim tool was then used to incorporate incidents into the simulation.
