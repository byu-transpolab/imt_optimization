...
```{r incident_capacity_restore, fig.width=24.57, fig.height=17.73}
...
score_data <- read_csv(score_output)
...
score_plot <- ggplot(score_data_long, aes(x = ITERATION, y = Score, color = Metric, group = Metric)) +
  geom_line(size = 2) +  # Increase the size for thicker lines
  labs(title = "MATSim Scores",
       x = "Iteration", 
       y = "Score") +
