...
\protect\hyperlink{sec-conclusions}{conclusions} provides an outline of
...
{[}Schultz, 2019; Skabardonis, 1998{]}. Furthermore, mesoscopic DTA
...
\chapter{Network Change Events}\label{network-change-events}}
...
\begin{Highlighting}[]
    \CommentTok{/**}
     \CommentTok{*}\NormalTok{ Creates and adds incident network change event to the scenario}\CommentTok{\textquotesingle{}}\NormalTok{s network}\CommentTok{.}
     \CommentTok{*}
     \CommentTok{*} \CommentTok{@}\NormalTok{param time       the time at which the change occurs}
     \CommentTok{*} \CommentTok{@}\NormalTok{param capacity   the reduced capacity value}
     \CommentTok{*} \CommentTok{@}\NormalTok{param linkId     the link to which the change applies}
     \CommentTok{*/}
    \KeywordTok{private} \DataTypeTok{void} \FunctionTok{createNetworkChangeEvent}\OperatorTok{(}\DataTypeTok{double}\NormalTok{ time}\OperatorTok{,} \DataTypeTok{double}\NormalTok{ capacity}\OperatorTok{,}\NormalTok{ Id}\OperatorTok{\textless{}}\NormalTok{Link}\OperatorTok{\textgreater{}}\NormalTok{ linkId}\OperatorTok{)} \OperatorTok{\{}
\NormalTok{        NetworkChangeEvent event }\OperatorTok{=} \KeywordTok{new} \FunctionTok{NetworkChangeEvent}\OperatorTok{(}\NormalTok{time}\OperatorTok{);}
\NormalTok{        event}\OperatorTok{.}\FunctionTok{setFlowCapacityChange}
        \OperatorTok{(}\KeywordTok{new} \FunctionTok{ChangeValue}\OperatorTok{(}\NormalTok{ChangeType}\OperatorTok{.}\FunctionTok{ABSOLUTE\_IN\_SI\_UNITS}\OperatorTok{,}\NormalTok{ capacity}\OperatorTok{));}
\NormalTok{        event}\OperatorTok{.}\FunctionTok{addLink}\OperatorTok{(}\NormalTok{scenario}\OperatorTok{.}\FunctionTok{getNetwork}\OperatorTok{().}\FunctionTok{getLinks}\OperatorTok{().}\FunctionTok{get}\OperatorTok{(}\NormalTok{linkId}\OperatorTok{));}
\NormalTok{        NetworkUtils}\OperatorTok{.}\FunctionTok{addNetworkChangeEvent}\OperatorTok{(}\NormalTok{scenario}\OperatorTok{.}\FunctionTok{getNetwork}\OperatorTok{(),}\NormalTok{ event}\OperatorTok{);}
    \OperatorTok{\}}
\end{Highlighting}
\end{Shaded}
